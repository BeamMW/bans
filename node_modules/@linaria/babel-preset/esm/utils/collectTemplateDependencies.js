...
export default function collectTemplateDependencies(babel, path, state, options) {
  const {
    types: t
  } = babel;
  ...
  const expressionValues = expressions.map(ex => {
    if (!ex.isExpression()) {
      throw ex.buildCodeFrameError(`The expression '${generator(ex.node).code}' is not supported.`);
    }
  ...
  debug('template-parse:evaluate-expressions', expressionValues.map(expressionValue => expressionValue.kind === ValueType.VALUE ? expressionValue.value : 'lazy'));
