...
function findWildcardReexportStatement(node, identifierName, graph) {
  ...
  if (!t.isProgram(program)) return null; // Our node is a correct export
  ...
  return program.body.find(statement => {
    ...
    if (!t.isCallExpression(object)) return false; // `object` should be `Object.keys`

    if (!t.isMemberExpression(object.callee) || !isIdentifier(object.callee.object, 'Object') || !isIdentifier(object.callee.property, 'keys')) return false; //
